...
import { Backdrop, Dialog, DialogContent, DialogContentText, DialogTitle, Menu, MenuItem, Modal } from '@mui/material'
...
const Notificacao = () => {
  ...
  return (
    <div>
        ...
        >
            <DialogTitle  className='bg-azulsite/30'>
                Próximas Consultas
            </DialogTitle>
            ...
            <DialogContent className='bg-azulsite/30 max-md:w-full'>
                    {user && user.pacienteId && data.length > 0 ? (
                        ...
                        <div className='mr-4'>
                            {consultas.profissional.areaAtuacao.areaId === 1 && (
                                <button className='bg-psi/30 py-1 px-4 rounded-md'>
                                    Entrar
                                </button>
                            )}
                            {consultas.profissional.areaAtuacao.areaId === 2 && (
                                <button onClick={() => handleChamada(consultas.profissional.profissionalId)} className='bg-nutri/30 py-1 px-4 rounded-md'>
                                    Entrar
                                </button>
                            )}
                        </div>
                    ...
                    <div className='bg-white rounded-md p-3 w-80'>
                        <p className='text-center'>
                            Não tem consultas disponíveis
                        </p>

                        <p className='text-center mt-5 mb-2'>Agende a sua agora com:</p>

                        <div className='flex justify-between bg-white'>
                        <Link to="/psicologia" className='bg-psi/50 py-1 px-3 rounded-md w-32 text-center ml-3'>
                            Psicólogos
                        </Link>
                        <Link to="/nutricao" className='bg-nutri/50 py-1 px-3 rounded-md w-32 text-center mr-3'>
                            Nutricionistas
                        </Link>
                        </div>
                    </div>
